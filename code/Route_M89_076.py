...
route_data = pd.read_csv("../data/google_transit_M89/20250327_google_transit_M89_076/routes.txt")

def route_populate(data):
    for index, row in data.iterrows():
        if(row['route_id'][:2]== "8_"):
            newRoute = ontology.InterurbanBusRoute("RouteM89_76_" + str(index+1))
        if(row['route_id'][:2]== "9_"):
            newRoute = ontology.UrbanBusRoute("RouteM89_76_" + str(index+1))
