...
data_calendar = pd.read_csv("../data/google_transit_M6/calendar.txt")
data_calendar_dates = pd.read_csv("../data/google_transit_M6/calendar_dates.txt")
...
def service_populate(calendar_data, calendar_dates_data):
    for index, row in calendar_data.iterrows():
        service = namespace.Service("ServiceM6_" + str(index+1))
