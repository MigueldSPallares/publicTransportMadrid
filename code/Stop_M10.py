...
if os.path.exists("../ontologies/publicTransportMadrid_M10.rdf"):
    ontology = get_ontology("../ontologies/publicTransportMadrid_M10.rdf").load()
else:
    ontology = get_ontology("../ontologies/publicTransportMadrid.rdf").load()

stop_data = pd.read_csv("../data/google_transit_M10/stops.txt")


def stop_populate(data):
    for index, row in data.iterrows():
        if(row['location_type']== 0):
            newStop = ontology.NormalStop("StopM10_" + str(index+1))
        elif(row['location_type']== 1):
            newStop = ontology.Station("StopM10_" + str(index+1))
        elif(row['location_type'] == 2):
            newStop = ontology.Entrance_Exit("StopM10_" + str(index+1))
        elif(row['location_type'] == 3):
            newStop = ontology.GenericNode("StopM10_" + str(index+1))
        elif(row['location_type'] == 4):
            newStop = ontology.BoardingArea("StopM10_" + str(index+1))
...
ontology.save(file = "../ontologies/publicTransportMadrid_M10.rdf")
