...
stop_time_data = pd.read_csv("../data/google_transit_M89/20250328_google_transit_M89_030/stop_times.txt")
namespace = get_namespace("http://vocab.gtfs.org/terms#")

def string_to_hour(time):
    if(time[:2]=="24"):
        time = time.replace(time[:2], "00")
    result = datetime.strptime(time, '%H:%M:%S').time()
    return result 

def stop_time_populate(data):
    for index, row in data.iterrows():

        newStopTime = namespace.StopTime("StopTimeM89_30_" + str(index + 1))
