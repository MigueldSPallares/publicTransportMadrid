...
stop_data = pd.read_csv("../data/google_transit_M5/stops.txt")


def stop_populate(data):
    for index, row in data.iterrows():
        if(row['location_type']== 0):
            newStop = ontology.NormalStop("StopM5_" + str(index+1))
        elif(row['location_type']== 1):
            newStop = ontology.Station("StopM5_" + str(index+1))
        elif(row['location_type'] == 2):
            newStop = ontology.Entrance_Exit("StopM5_" + str(index+1))
        elif(row['location_type'] == 3):
            newStop = ontology.GenericNode("StopM5_" + str(index+1))
        elif(row['location_type'] == 4):
            newStop = ontology.BoardingArea("StopM5_" + str(index+1))
